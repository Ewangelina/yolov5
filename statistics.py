...
def analise_line(textLine): #returns True if there are motorcycles in frame
    global sum_no_motorcycles, prev_no_motorcycles, prev_prev_no_motorcycles, frames_of_same_detection, prev_frames_of_same_detection
    
    parts = textLine.split(" ")
    index = -1
    for i in range(len(parts)):
        if parts[i] == "motorcycle":
            index = i - 1
            break
    ...
    if index == -1: #No motorcycles
        ...
        if prev_no_motorcycles == no_motorcycles:
            ...
            if prev_prev_no_motorcycles == no_motorcycles and frames_of_same_detection <= 1: #motorcycle returned
                frames_of_same_detection += 1
            else:
                sum_no_motorcycles += prev_no_motorcycles - no_motorcycles
                prev_prev_no_motorcycles = prev_prev_no_motorcycles
                frames_of_same_detection = 0
